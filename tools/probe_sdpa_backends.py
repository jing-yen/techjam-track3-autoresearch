...
of FLASH / EFFICIENT_ATTENTION / MATH via torch.nn.attention.sdpa_kernel,
recording which ones the shape is even eligible for. PyTorch's real (default,
unforced) dispatch always picks the highest-priority eligible backend in
that same order, so "first eligible" here is what actually fires -- no need
to trace kernel launches.
...
def main() -> int:
    ...
    for sid in ids:
        ...
        try:
            ...
            fires = next((n for n in ("flash", "mem_efficient", "math") if elig.get(n)), "none")
        ...
        print(f"shape {sid:>2}  head_dim={head_dim:>4}  fires={fires:<14}  "
              f"flash={elig.get('flash')}  mem_eff={elig.get('mem_efficient')}  math={elig.get('math')}")
