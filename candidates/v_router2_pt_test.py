"""
v_router2_autotuned -- v_router2.py + T7b: @triton.autotune on the shared
`_fused_add_layernorm_kernel` (num_warps/num_stages, key=["n_cols"]).
...
# --------------------------------------------------------------------------- #
# PILOT, NOT YET ROUTED BY DEFAULT: pre-transposed Linear weights (U5,
# candidates/v_pretransposed.py) applied to the "amp" and "fusedcg" routes
# specifically -- the two eager (non-torch.compile) routes where cuBLAS
# kernel-layout selection is directly under our control. New classes, not
# modifications of _BestAttention/_FusedAttention/etc, so "compile"/"reduce"
# (which reuse those same shared classes) are completely unaffected -- same
# isolation discipline as every other route split in this file.
#
# nn.Linear computes x @ W.T + b every call (cuBLAS "NT" layout). Storing the
# weight pre-transposed ([in,out] instead of [out,in]) and computing x @ W + b
# directly gives cuBLAS its "NN" layout instead -- generally the best-covered,
# most-optimized GEMM case. Standalone-validated: 1.80x/1.84x vs plain
# baseline (median/geomean, RunPod A100-80). NOT yet validated stacked on top
# of the CUDA-graph-capturing amp/fusedcg routes -- that's what this pilot
# file tests, temporarily overriding _ROUTE below for shapes 6/8/9/10/11/12
# only, so results are directly A/B-comparable against v_router2_autotuned.py
# on the exact same shapes before any route table is permanently changed.
# --------------------------------------------------------------------------- #
...
class _BestAttentionPT(nn.Module):
    def __init__(self, d_model: int, num_heads: int) -> None:
        super().__init__()
        self.d_model = d_model
        self.num_heads = num_heads
...
class _BestBlockTriton2PT(nn.Module):
    def __init__(self, d_model, num_heads, ffn_dim):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.attention = _BestAttentionPT(d_model, num_heads)
        self.norm2 = nn.LayerNorm(d_model)
...
class _BestTransformer2PT(BaselineTransformer):
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config)
        opt_layers = nn.ModuleList()
        for _ in self.layers:
            opt_layers.append(_BestBlockTriton2PT(config.d_model, config.num_heads, config.ffn_dim))
        self.layers = opt_layers
...
class _AMPTransformerPT(_BestTransformer2PT):
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config)
        self._graph = None
        self._static_x = None
...
class _FusedAttentionPT(nn.Module):
    def __init__(self, d_model: int, num_heads: int) -> None:
        super().__init__()
        self.d_model = d_model
        self.num_heads = num_heads
        self.head_dim = d_model // num_heads
        self.qkv = PreTransposedLinear(d_model, 3 * d_model)
        self.out_proj = PreTransposedLinear(d_model, d_model)
...
class _FusedBlockAddNorm2PT(nn.Module):
    def __init__(self, d_model, num_heads, ffn_dim):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.attention = _FusedAttentionPT(d_model, num_heads)
        self.norm2 = nn.LayerNorm(d_model)
        self.ffn_in = PreTransposedLinear(d_model, ffn_dim)
        self.ffn_out = PreTransposedLinear(ffn_dim, d_model)
...
class _FusedTransformerCudaGraphPT(BaselineTransformer):
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config)
        opt_layers = nn.ModuleList()
        for _ in self.layers:
            opt_layers.append(_FusedBlockAddNorm2PT(config.d_model, config.num_heads, config.ffn_dim))
        self.layers = opt_layers
        self._graph = None
        self._static_x = None
        self._static_out = None
...
def _pretransposed_copy(baseline, optimized, fused_qkv: bool) -> None:
    """Like _fused_copy (for fused_qkv=True) or a strict per-name copy
    (fused_qkv=False), but transposes every Linear weight [out,in] -> [in,out]
    at copy time to match PreTransposedLinear's storage layout."""
    ...
    if fused_qkv:
        for i in range(len(optimized.layers)):
            ...
            new[p + "qkv.weight"] = qkv_w.t().contiguous()
            new[p + "qkv.bias"] = torch.cat(
                [b[p + "q_proj.bias"], b[p + "k_proj.bias"], b[p + "v_proj.bias"]], dim=0)
        attn_linear_suffix = {"out_proj"}
    else:
        attn_linear_suffix = {"q_proj", "k_proj", "v_proj", "out_proj"}

    for i in range(len(optimized.layers)):
        p = f"layers.{i}."
        for name in ("norm1.weight", "norm1.bias", "norm2.weight", "norm2.bias"):
            new[p + name] = b[p + name]
        for lin in attn_linear_suffix:
            key = p + "attention." + lin
            if key + ".weight" not in new:
                new[key + ".weight"] = b[key + ".weight"].t().contiguous()
                new[key + ".bias"] = b[key + ".bias"]
        for lin in ("ffn_in", "ffn_out"):
            key = p + lin
            new[key + ".weight"] = b[key + ".weight"].t().contiguous()
            new[key + ".bias"] = b[key + ".bias"]
    new["final_norm.weight"] = b["final_norm.weight"]
    new["final_norm.bias"] = b["final_norm.bias"]
...
_IMPLS = {
    ...
    "amp_pt": _AMPTransformerPT,
    "fusedcg_pt": _FusedTransformerCudaGraphPT,
}
...
_ROUTE = {
    ...
    (10000, 128, 128, 4): "amp_pt",  # shape 6  -- PILOT: pretransposed Linears on the amp route.
    ...
    (64, 128, 1024, 4):  "amp_pt",   # shape 8  -- PILOT, see shape 6's note.
    (64, 128, 128, 1):   "fusedcg_pt",  # shape 9  -- PILOT: pretransposed Linears on the fusedcg route.
                                      # Was "fusedcg"; see shape 6's note. Original T17 evidence
                                      # (job 779394): 3.55x vs plain fused's 2.23x (+59.1%) -- AddNorm
    ...
    (64, 128, 128, 2):   "fusedcg_pt",  # shape 10 -- PILOT, see shape 9's note.
    (64, 128, 128, 16):  "fusedcg_pt",  # shape 11 -- PILOT, see shape 9's note.
    (64, 32, 128, 4):    "fusedcg_pt",  # shape 12 -- PILOT, see shape 9's note.
...
def copy_model_weights(baseline, optimized: "UserOptimizedTransformer") -> None:
    ...
    elif optimized._impl_name == "amp_pt":
        _pretransposed_copy(baseline, optimized._impl, fused_qkv=False)
    elif optimized._impl_name == "fusedcg_pt":
        _pretransposed_copy(baseline, optimized._impl, fused_qkv=True)
    else:
