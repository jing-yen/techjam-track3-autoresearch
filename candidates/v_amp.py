"""
T6 -- fp16 via torch.autocast, not a blanket dtype cast.

Motivation: tools/probe_sdpa_backends.py showed flash SDPA becomes eligible
on ALL 14 official shapes at fp16 (vs zero at fp32 -- see B2/leaderboard.md),
including #8 and #14, which older docs suggested were over sm80's head_dim
limit. But `runner.py --dtype float16` (blanket cast: weights, activations,
LayerNorm, GELU, everything to fp16) failed correctness on 11/12 shapes --
max_abs ~0.006-0.009, and catastrophic max_rel on several, consistent with
LayerNorm/FFN precision loss, not just attention.

This candidate keeps model parameters and the input in fp32 (run with
--dtype float32, same as every other candidate) and wraps only the forward
pass in torch.autocast(dtype=float16). Autocast's per-op policy runs
matmul/attention in fp16 (unlocking flash) while keeping precision-sensitive
reductions -- LayerNorm, softmax -- in fp32 automatically, rather than us
hand-picking casts. This is the standard mixed-precision recipe; the naive
full-cast experiment specifically motivates why it's necessary here rather
than optional.

On CPU (no autocast fp16 support), falls back to plain fp32 SDPA -- same
as best.py -- so correctness/dev tests still run without CUDA.
"""
