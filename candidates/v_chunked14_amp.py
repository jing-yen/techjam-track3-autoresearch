"""
S5+T6 -- batch-chunked forward pass under fp16 autocast, for shape #14.
...
so splitting into groups and concatenating is exact, not approximate). This
variant additionally wraps each chunk's forward in torch.autocast(fp16)
(T6, confirmed correctness-safe up to max_abs ~0.0017 on the shapes that have
a reference -- see journal iter 19's full 13-shape AMP sweep).

Motivation for combining them: fp16 activations are roughly half the memory
of fp32, so the same physical memory budget should support a LARGER chunk
size than v_chunked14.py's chunk=4 -- fewer, bigger forward passes, less
Python/launch overhead per element. Set chunk=8 here (double v_chunked14.py's
4) as a first test of that headroom; if it holds well under the 79.25 GB
ceiling, a future pass can push it further.
