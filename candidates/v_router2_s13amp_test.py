...
_ROUTE = {
    ...
    (64, 1024, 128, 4):  "amp",      # shape 13 -- PILOT: isolated single-shape test measured
                                      # 14.08x for amp vs ~4.03-4.09x for the current "compile"
                                      # fallback. Testing INSIDE the full 1-13 sweep here, not
                                      # isolated, given the established CUDA-graph-presence side
                                      # effect (iter 54) where an isolated number didn't transfer
                                      # once other CUDA-graph shapes (9-12, fusedcg) were also in
                                      # the same process/sweep (13.15x isolated -> 5.46x in-sweep).
