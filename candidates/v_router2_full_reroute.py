"""
v_router2_autotuned -- v_router2.py + three stacked, individually-verified
...
routed to "amp" instead of its previous "compile" fallback -- 4.03x -> 14.13x,
the largest single win in the project, GPU-confirmed inside the full 1-13
sweep specifically to rule out the iter-54 CUDA-graph-presence side effect
(it did not reproduce here). Aggregate: median 4.57x (plain v_router2.py)
-> 5.71x, geomean 4.85x -> 5.42x, 13/13 correct. See the _ROUTE table entries
for shapes 13/14 and journal.jsonl iter 55/57 for full evidence.
...
_ROUTE = {
    ...
    (64, 128, 128, 4):   "amp",      # shape 1  -- was "compile" (2.60x); amp 4.84x (+85.9%)
    ...
    (128, 128, 128, 4):  "amp",      # shape 5  -- was "reduce" (2.77x); amp 3.64x (+31.7%)
    (10000, 128, 128, 4): "amp",     # shape 6  -- confirmed still best (4.05x)
    (64, 128, 32, 4):    "amp",      # shape 7  -- was "compile" (5.84x); amp 7.58x (+29.9%)
    (64, 128, 1024, 4):  "amp",      # shape 8  -- confirmed still best (1.80x)
    ...
    (64, 128, 128, 16):  "amp",      # shape 11 -- was "fusedcg" (3.45x); amp 5.37x (+55.6%)
    (64, 32, 128, 4):    "fusedcg",  # shape 12 -- confirmed still best (11.09x)
    (64, 1024, 128, 4):  "amp",      # shape 13 -- was falling through to "compile"
