...
class _FusedCompileTransformer(_FusedTransformer):
    """Fused-QKV route compiled as one graph for the large layout-heavy cases.

    The eager fused route already saves two projection launches, but leaves the
    QKV views, BHSD/BSHD transpose and surrounding pointwise work visible as
    separate eager operations.  Compiling the full layer stack gives Inductor a
    chance to keep those views symbolic and remove redundant layout work while
    preserving the already-validated fused parameter layout.
    """
...
_IMPLS = {
    ...
    "fused_compile": _FusedCompileTransformer,
}
...
_ROUTE = {
    ...
    (64, 128, 1024, 4):  "fused_compile", # shape 8  -- T4: fused QKV + compiled layout graph
    ...
    (64, 1024, 128, 4):  "fused_compile", # shape 13 -- T4: fused QKV + compiled layout graph
}
_FALLBACK = "compile"


def copy_model_weights(baseline, optimized: "UserOptimizedTransformer") -> None:
    if optimized._impl_name in ("fused", "fused_compile"):
